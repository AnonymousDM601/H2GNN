...
class Experiment:
    def __init__(self, args):
        ...
        self.kwargs = {"hidden_drop":args.hidden_drop, "input_drop":args.input_drop}
        self.hyperpars = {"lr":args.lr,"emb_dim":args.emb_dim,"nr":args.nr, "hidden_drop":args.hidden_drop, "input_drop":args.input_drop}
    ...
    def get_model(self):
        ...
        """
        model = H2GCN(self.dataset, self.emb_dim, self.c, args).to(self.device)
        return model
    ...
    def test_and_eval(self):
        print("Testing the model on {}...".format(self.dataset.name))
        self.model.eval()
        with torch.no_grad():
            tester = Tester(self.device, self.dataset, self.model, "test")
    ...
    def train_and_eval(self):
        ...
        for it in range(self.model.cur_itr.data, self.num_iterations+1):
            ...
            iter_time = time.time() - time_s
            print("Iteration#: {}, loss: {}, time: {}".format(it, losses, iter_time))

            # Evaluate the model every 100th iteration or if it is the last iteration
            if (it % 100 == 0 and it!=0) or (it == self.num_iterations):
                self.model.eval()
                with torch.no_grad():
                    print("validation:")
                    tester = Tester(self.device, self.dataset, self.model, "valid")
        ...
        with torch.no_grad():
            ...
            tester = Tester(self.device, self.dataset, self.best_model, "test")
            self.measure, self.measure_by_arity = tester.test(self.test_by_arity)
    ...
    def create_output_dir(self, output_dir=None):
        ...
        if output_dir is None:
            ...
            time_name = '{}'.format(time.strftime("%Y%m%d-%H%M%S"))
            output_dir = os.path.join(DEFAULT_SAVE_DIR, self.dataset.name, time_name)
    ...
    def save_model(self, itr=0, test_or_valid='test', is_best_model=False):
            ...
            opt_name = 'opt_{}itr.chkpnt'.format(itr) if itr else '{}.chkpnt'.format('H2GCN')
            measure_name = '{}_measure_{}itr.json'.format(test_or_valid, itr) if itr else '{}.json'.format('H2GCN')
            ...
            if (self.test_by_arity) and (self.measure_by_arity is not None):
                H = {}
                measure_by_arity_name = '{}_measure_{}itr_by_arity.json'.format(test_or_valid, itr) if itr else '{}.json'.format('H2GCN')
